...
const TOKEN = 'eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpZCI6MTAwMDAsIm5hbWUiOiJVc2VyICMxIiwiZW1haWwiOiJ1c2VyMUBlbWFpbC5jb20ifQ.GaDSr7O2IQcY-RzG3g3-a9aONJ-l9OWT01bxaZw0Ujg';
...
describe('Ao salvar uma transferencia válida...', () => {
  ...
  test('Deve retrnar o satus 201 e os dados da transferencia', () => {
    return request(app).post(MAIN_ROUTE)
      .set('authorization', `bearer ${TOKEN}`)
      .send({
        description: 'Regular Transfer', user_id: 10000, acc_ori_id: 10000, acc_dest_id: 10001, ammount: 100, date: new Date(),
...
describe('Ao tentar salvar uma transferencua inválida....', () => {
  const validTransfer = {
    description: 'Regular Transfer', user_id: 10000, acc_ori_id: 10000, acc_dest_id: 10001, ammount: 100, date: new Date(),
  };
  ...
  test('Não deve inserir se as contas de origem e destino forem as mesmas', () => template({ acc_dest_id: 10000 }, 'Conta de Destino não pode ser a mesma da Conta de Origem'));
  test('Não deve inserir se as contas pertencerem a ourto usuario', () => template({ acc_ori_id: 20000 }, 'Conta #20000 não pertence ao usuário'));
});
...
describe('Ao alterar uma transferencia válida...', () => {
  ...
  test('Deve retrnar o satus 200 e os dados da transferencia', () => {
    return request(app).put(`${MAIN_ROUTE}/10000`)
      .set('authorization', `bearer ${TOKEN}`)
      .send({
        description: 'Transfer Update', user_id: 10000, acc_ori_id: 10000, acc_dest_id: 10001, ammount: 500, date: new Date(),
      })
      .then(async (res) => {
        expect(res.status).toBe(200);
  ...
  test('Ambas devem estar com status de realizado', () => {
    expect(income.status).toBe(true);
    expect(outcome.status).toBe(true);
  });
});

describe('Ao tentar alterar uma transferencua inválida....', () => {
  const validTransfer = {
    description: 'Update Transfer', user_id: 10000, acc_ori_id: 10000, acc_dest_id: 10001, ammount: 100, date: new Date(),
  };
